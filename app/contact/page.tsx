...
import { spring, tEnter, tReveal } from '@/lib/motion';
...
export default function Contact() {
  ...
  if (isSubmitted) {
    return (
      <div className="relative min-h-screen overflow-hidden">
        ...
        <div className="relative z-10 flex min-h-screen flex-col items-center justify-center px-[max(1rem,env(safe-area-inset-left,0px))] pb-20 pt-[max(7rem,env(safe-area-inset-top,0px)+5.5rem)] pr-[max(1rem,env(safe-area-inset-right,0px))] sm:px-6">
          <SiteCenter max="md">
          <motion.div
  ...
  const fieldClass =
    'w-full rounded-lg border border-white/[0.08] bg-white/[0.02] px-3.5 py-2.5 text-left text-base text-white placeholder:text-slate-600 transition-[border-color,box-shadow] duration-300 ease-smooth focus:border-emerald-500/45 focus:outline-none focus:ring-1 focus:ring-emerald-500/25';
                ...
                <form onSubmit={handleSubmit} className="mx-auto mt-8 max-w-xl space-y-6 text-center">
                  ...
                  <div className="flex justify-center">
                    ...
                    >
                      {isSubmitting ? (
                        <>
                          <div className="h-5 w-5 animate-smooth-spin rounded-full border-2 border-white border-t-transparent" />
                          <span>Sending…</span>
                        </>
                      ) : (
                        <>
