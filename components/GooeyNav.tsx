...
const GooeyNav = ({
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>, index: number) => {
    ...
    // Handle scroll to services section if it's the Services link
  ...
  // Handle route changes - clear active state when pathname changes
  useEffect(() => {
    setActiveIndex(-1);
    if (textRef.current) {
      textRef.current.classList.remove('active');
  ...
  useEffect(() => {
    ...
    // Only update effect position if there's an active index (not -1)
    if (activeIndex >= 0) {
      ...
      // Clear active state when activeIndex is -1
      textRef.current?.classList.remove('active');
    }
