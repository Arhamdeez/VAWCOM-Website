...
import { tAmbient } from '@/lib/motion';
...
export default function PremiumPageBackdrop() {
  ...
  const [staticOrbs, setStaticOrbs] = useState(true);

  useEffect(() => {
    const mobile = window.matchMedia('(max-width: 767px)').matches;
    const coarse = window.matchMedia('(pointer: coarse)').matches;
    setStaticOrbs(Boolean(reduceMotion) || mobile || coarse);
  }, [reduceMotion]);
        ...
        {staticOrbs ? (
          ...
          <>
            ...
            <motion.div
              ...
              animate={{ scale: [1, 1.14, 1], x: [0, -28, 0], y: [0, -18, 0] }}
